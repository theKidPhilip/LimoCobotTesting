...
class WallFollowerPD(Node):
    ...
    lin_vel_zero = 0.000
    lin_vel_slow = 0.100
    lin_vel_fast = 0.200          # slightly lower than original for PD stability
    ang_vel_zero = 0.000
    ang_vel_max = 0.500          # hard cap on angular output
    ...
    def _follow_wall_pd(self):
        ...
        if self.side_chosen == "right":
            ...
            side_range = self.scan_left_range

        # ── compute PD terms ─────────────────────────────────────
        # error > 0  → too far from wall  → need to steer toward wall
        # error < 0  → too close to wall  → need to steer away from wall
        ...
        # ── sign convention ───────────────────────────────────────
        # Right wall: positive pd_output → steer right (negative angular.z)
        # Left  wall: positive pd_output → steer left  (positive angular.z)
        if self.side_chosen == "right":
            angular_correction = -pd_output
        else:
            angular_correction = pd_output
        ...
        if self.scan_front_range < self.front_threshold:
            ...
            self.twist_cmd.linear.x = self.lin_vel_slow
            if self.side_chosen == "right":
                self.twist_cmd.angular.z = self.ang_vel_max   # turn left
            else:
                self.twist_cmd.angular.z = -self.ang_vel_max   # turn right
        else:
            self.twist_cmd.linear.x = self.lin_vel_fast
            self.twist_cmd.angular.z = angular_correction

        self.get_logger().info(
            "PD | err=%+.4f  deriv=%+.4f  out=%+.4f  ang=%+.4f" %
            (error, derivative, pd_output, self.twist_cmd.angular.z))
